...
def main(_):
    ...
    for ckpt_path in tf.train.checkpoints_iterator(
        ...
        for scene, flare in tf.data.Dataset.zip((scenes, flares)):
            ...
            scene_batch = summary[:, :, 512: 768, :]
            predict_batch = summary[:, :, 256: 512, :]

            batch_psnr = tf.image.psnr(scene_batch, predict_batch, max_val=1.0)
            total_psnr += tf.reduce_sum(batch_psnr).numpy()

            batch_ssim = tf.image.ssim(scene_batch, predict_batch, max_val=1.0)
            total_ssim += tf.reduce_sum(batch_ssim).numpy()

            batch_counter += batch_psnr.shape[0]

        avg_psnr = total_psnr / batch_counter if batch_counter > 0 else 0.0
        average_ssim = total_ssim / batch_counter if batch_counter > 0 else 0.0

        with summary_writer.as_default():
            tf.summary.image('prediction', summary, max_outputs=1, step=ckpt.step)
            tf.summary.scalar('loss', loss_value, step=ckpt.step)
            tf.summary.scalar("average_psnr", avg_psnr, step=batch_counter)
            tf.summary.scalar("average_ssim", average_ssim, step=batch_counter)
